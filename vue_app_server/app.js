...
app.get("/getImages",(req,res)=>{
  var rows = [
    ...
    {id:3,img_url:"http://127.0.0.1:3000/img/lunbo3.jpg"},
  ];
  res.send(rows);
})
...
app.get("/getBooksInfo",(req,res)=>{
  //1:参数   id   1
  var id = req.query.id;
  var sql=" SELECT id,name,title,author,book_url";
      sql+=" FROM xz_book WHERE id = ?";
  //3:json   {code:1,data:obj} 
  pool.query(sql,[id],(err,result)=>{
      if(err)throw err;
      res.send({code:1,data:result[0]});
...
app.get("/getBComments",(req,res)=>{
  ...
  //3:创建sql
  //  查询总页数
  var sql = "SELECT count(id) as c FROM xz_bcomment";
  sql +=" WHERE nid = ?"
  var progress = 0; //sql执行进度
  obj = {code:1};
  pool.query(sql,[nid],(err,result)=>{
...
//功能十三：同步购物车中商品数量
app.get("/updataCart",(req,res)=>{
  var id=req.parseInt(req.query.id);
  var count=req.parseInt(req.query.count);
  
})

app.get("/Logout",(req,res)=>{
  req.seccion.uid=null;
  res.send({code:1,msg:"退出成功"});

})
//查询书籍信息   
app.get("/getBookImages",(req,res)=>{
  //2:sql
  var sql=" SELECT id,name,title,book_url,author";
  sql +=" FROM xz_book";
  pool.query(sql,(err,result)=>{
      if(err)throw err;
      res.send({result});
...
//书架的图书插入
app.get("/getBookImages2",(req,res)=>{
    var sql=" SELECT id,name,book_url FROM xz_book2";
  pool.query(sql,(err,result)=>{
    if(err)throw err;
     res.send({result});
  })
